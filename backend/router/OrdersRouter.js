...
router.post('/addorder',
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (existingOrder) {
        if (existingOrder.qty >= product.stock) {
          return res.status(422).json({ message: 'Order quantity exceeds available stock', status: 2 });
        }
        else{
          existingOrder.qty++;
        ...
        res.send({ message: 'Out of Stuck', status: 2, });
      }
    } catch (err) {
      console.error(err);
...
router.put('/editOrderStatus/:id', async (req, res) => {
  ...
  try {
    const order = await OrdersModel.findById(id).populate('product_id'); // Populate product details

    if (!order) {
      return res.status(404).json({ message: 'Order not found' });
    }

    if (status === 4) { // Update stock only when status is 4
      const product = order.product_id; // Access populated product info
      const newStock = product.stock - order.qty; // Calculate new stock

      if (newStock < 0) { // Handle insufficient stock scenario
    ...
    order.status = status;
    await order.save(); // Update order status
